...
func LoginHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method == "POST" {
		...
		if util.ErrorCheckHandlers(w, "Invalid credentials", err, http.StatusUnauthorized) {
			return
		}

		if err := session.CreateSession(w, userID); util.ErrorCheckHandlers(w, "Session creation failed", err, http.StatusInternalServerError) {
			return
		}
